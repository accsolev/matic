...
class CekBpjs {
    ...
    async getBillDetails(customerId, voucherId) {
        ...
        try {
            const inquiryUrl = `https://` + CekBpjs.API_HOST + `/api/v2/order`;
            ...
            const inquiryResponse = await axios.post(inquiryUrl, inquiryPayload, { 
                headers: { 
                    'Host': CekBpjs.API_HOST,
                    'User-Agent': CekBpjs.USER_AGENT,
        ...
        const detailsUrl = `https://` + CekBpjs.API_HOST + `/api/v2/get`;
