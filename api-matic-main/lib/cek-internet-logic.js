...
class CekInternet {
    ...
    async getBillDetails(customerId, voucherId) {
        const staticParams = {
        ...
        try {
            const inquiryUrl = `https://` + CekInternet.API_HOST + `/api/v2/order`;
            ...
            const inquiryResponse = await axios.post(inquiryUrl, inquiryPayload, { 
                headers: { 
                    'Host': CekInternet.API_HOST,
                    'User-Agent': CekInternet.USER_AGENT,
        ...
        const detailsUrl = `https://` + CekInternet.API_HOST + `/api/v2/get`;
