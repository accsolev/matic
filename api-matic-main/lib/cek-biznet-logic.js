...
class CekBiznet {
    ...
    async getBillDetails(customerId) {
        ...
        try {
            const inquiryUrl = `https://` + CekBiznet.API_HOST + `/api/v2/order`;
            ...
            const inquiryResponse = await axios.post(inquiryUrl, inquiryPayload, { 
                headers: { 
                    'Host': CekBiznet.API_HOST,
                    'User-Agent': CekBiznet.USER_AGENT,
        ...
        const detailsUrl = `https://` + CekBiznet.API_HOST + `/api/v2/get`;
