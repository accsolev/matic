...
class CekIconnect {
    ...
    async getBillDetails(customerId) {
        ...
        try {
            const inquiryUrl = `https://` + CekIconnect.API_HOST + `/api/v2/order`;
            ...
            const inquiryResponse = await axios.post(inquiryUrl, inquiryPayload, { 
                headers: { 
                    'Host': CekIconnect.API_HOST,
                    'User-Agent': CekIconnect.USER_AGENT,
        ...
        const detailsUrl = `https://` + CekIconnect.API_HOST + `/api/v2/get`;
