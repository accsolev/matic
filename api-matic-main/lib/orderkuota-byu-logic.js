...
class OrderKuotaByu {
    ...
    async getByuProducts(authToken, authUsername) {
        const url = `https://` + OrderKuotaByu.API_HOST + `/api/v2/get`;
        const bodyPayload = new URLSearchParams({
            ...
            'requests[vouchers][product]': 'kuota_byu',
        ...
        const headers = {
            'Host': OrderKuotaByu.API_HOST,
            'User-Agent': OrderKuotaByu.USER_AGENT,
        ...
        try {
            ...
            throw new Error(`Failed to get by.U products: ${errorMessage}`);
        }
    }

    async orderByu(authToken, authUsername, voucherId, targetNumber) {
        const url = `https://` + OrderKuotaByu.API_HOST + `/api/v2/order`;
        ...
        try {
            ...
            const errorMessage = error.response ? JSON.stringify(error.response.data) : error.message;
            throw new Error(`Failed to place by.U order: ${errorMessage}`);
        }
    }
}

module.exports = { OrderKuotaByu };
