...
class OrderKuotaPulsa {
    static API_HOST = 'app.orderkuota.com';
    static USER_AGENT = 'okhttp/4.12.0';

    async getPulsaProducts(authToken, authUsername) {
        const url = `https://` + OrderKuotaPulsa.API_HOST + `/api/v2/get`;
        ...
        const headers = {
            'Host': OrderKuotaPulsa.API_HOST,
            'User-Agent': OrderKuotaPulsa.USER_AGENT,
        ...
        try {
            ...
            throw new Error(`Failed to get pulsa products: ${errorMessage}`);
        }
    }

    // --- FUNGSI BARU UNTUK MELAKUKAN ORDER ---
    async orderPulsa(authToken, authUsername, voucherId, targetNumber) {
        const url = `https://` + OrderKuotaPulsa.API_HOST + `/api/v2/order`;
        ...
        try {
            ...
            const errorMessage = error.response ? JSON.stringify(error.response.data) : error.message;
            throw new Error(`Failed to place pulsa order: ${errorMessage}`);
        }
    }
}

module.exports = { OrderKuotaPulsa };
