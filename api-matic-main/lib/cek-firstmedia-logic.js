...
class CekFirstMedia {
    ...
    async getBillDetails(customerId) {
        ...
        try {
            const inquiryUrl = `https://` + CekFirstMedia.API_HOST + `/api/v2/order`;
            ...
            const inquiryResponse = await axios.post(inquiryUrl, inquiryPayload, { 
                headers: { 
                    'Host': CekFirstMedia.API_HOST,
                    'User-Agent': CekFirstMedia.USER_AGENT,
        ...
        const detailsUrl = `https://` + CekFirstMedia.API_HOST + `/api/v2/get`;
