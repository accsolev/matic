...
class OrderKuotaCicilan {
    static API_HOST = 'app.orderkuota.com';
    static USER_AGENT = 'okhttp/4.12.0';

    async getCicilanProducts(authToken, authUsername) {
        const url = `https://` + OrderKuotaCicilan.API_HOST + `/api/v2/get`;
        const bodyPayload = new URLSearchParams({
            ...
            'auth_username': authUsername, 'requests[vouchers][product]': 'finance', 'auth_token': authToken,
            'app_version_name': '25.03.27', 'ui_mode': 'dark', 'requests[0]': 'balance', 'phone_model': 'SM-G935F'
        }).toString();
        const headers = { 'Host': OrderKuotaCicilan.API_HOST, 'User-Agent': OrderKuotaCicilan.USER_AGENT, 'Content-Type': 'application/x-www-form-urlencoded' };
        try {
            const response = await axios.post(url, bodyPayload, { headers });
            return { success: true, ...response.data };
        } catch (error) {
            const errorMessage = error.response ? JSON.stringify(error.response.data) : error.message;
            throw new Error(`Failed to get cicilan products: ${errorMessage}`);
        }
    }

    async orderCicilan(authToken, authUsername, voucherId, targetCustomerId) {
        const staticParams = {
            'app_reg_id': 'fR1V5tOkS729uGKf5CwnEX:APA91bGZonDueIesJj2EleKqqtlpTqXGj20YHHftevWl4-6aE04yGfKZCyqfcvyqxLoLsC4RV9qhcO3ZCL7VeovaWSF_QS25ao0SPU-C3BCO0t_MPLfBl7Y',
        ...
        try {
            const orderUrl = `https://` + OrderKuotaCicilan.API_HOST + `/api/v2/order`;
            ...
            const orderResponse = await axios.post(orderUrl, orderPayload, { 
                headers: { 'Host': OrderKuotaCicilan.API_HOST, 'User-Agent': OrderKuotaCicilan.USER_AGENT, 'Content-Type': 'application/x-www-form-urlencoded' }
            });
            ...
            throw new Error(`Langkah 1 (Order) gagal: ${error.response ? JSON.stringify(error.response.data) : error.message}`);
        }

        const detailsUrl = `https://` + OrderKuotaCicilan.API_HOST + `/api/v2/get`;
...
module.exports = { OrderKuotaCicilan };
