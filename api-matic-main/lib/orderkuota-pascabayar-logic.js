...
class OrderKuotaPascabayar {
    ...
    async getPascabayarProducts(authToken, authUsername) {
        const url = `https://` + OrderKuotaPascabayar.API_HOST + `/api/v2/get`;
        const bodyPayload = new URLSearchParams({
            ...
            'requests[vouchers][product]': 'pascabayar',
        ...
        try {
            ...
            throw new Error(`Failed to get pascabayar products: ${errorMessage}`);
        }
    }
    ...
    async checkBill(authToken, authUsername, voucherId, targetNumber) {
        ...
        try {
            ...
            const orderResponse = await axios.post(orderUrl, orderPayload, { 
                headers: { 'Host': OrderKuotaPascabayar.API_HOST, 'User-Agent': OrderKuotaPascabayar.USER_AGENT, 'Content-Type': 'application/x-www-form-urlencoded' }
            });
