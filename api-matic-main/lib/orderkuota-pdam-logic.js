...
class OrderKuotaPdam {
    static API_HOST = 'app.orderkuota.com';
    static USER_AGENT = 'okhttp/4.12.0';

    async getPdamProducts(authToken, authUsername) {
        const url = `https://` + OrderKuotaPdam.API_HOST + `/api/v2/get`;
        const bodyPayload = new URLSearchParams({
            ...
            'requests[vouchers][product]': 'air_pdam',
        ...
        const headers = {
            'Host': OrderKuotaPdam.API_HOST,
            'User-Agent': OrderKuotaPdam.USER_AGENT,
        ...
        try {
            ...
            throw new Error(`Failed to get PDAM products: ${errorMessage}`);
        }
    }
    ...
    async checkBill(authToken, authUsername, voucherId, targetCustomerId) {
        ...
        let transactionId;
        try {
            const orderUrl = `https://` + OrderKuotaPdam.API_HOST + `/api/v2/order`;
            ...
            const orderResponse = await axios.post(orderUrl, orderPayload, { 
                headers: { 'Host': OrderKuotaPdam.API_HOST, 'User-Agent': OrderKuotaPdam.USER_AGENT, 'Content-Type': 'application/x-www-form-urlencoded' }
            });
            ...
            throw new Error(`Langkah 1 (Check Bill) gagal: ${error.response ? JSON.stringify(error.response.data) : error.message}`);
        }

        const detailsUrl = `https://` + OrderKuotaPdam.API_HOST + `/api/v2/get`;
