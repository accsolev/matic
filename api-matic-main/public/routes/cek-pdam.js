...
function formatPdamResponse(transactionData) {
...
module.exports = async function(req, res) {
    try {
        ...
        const result = await pdamApi.checkBill(authToken, authUsername, voucher_id, target_customer_id);

        if (result.success && result.transaction_details) {
            const finalResponse = formatPdamResponse(result.transaction_details);
