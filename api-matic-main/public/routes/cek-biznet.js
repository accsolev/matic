const { CekBiznet } = require('../../lib/cek-biznet-logic.js');
...
function formatBiznetResponse(result) {
    if (!result || !result.body?.["Rincian Transaksi"] || !result.top) {
        return { success: false, message: "Struktur data transaksi Biznet tidak valid atau tidak ditemukan." };
    }
    ...
    if (sn) {
        ...
        return {
            success: true,
            status: "Sukses",
            message: "Tagihan Biznet berhasil ditemukan.",
            bill_details: {
...
module.exports = async function(req, res) {
    try {
        ...
        const biznetApi = new CekBiznet();
        const result = await biznetApi.getBillDetails(customer_id);
        
        const finalResponse = formatBiznetResponse(result.details.content_details);
        ...
        console.error("Biznet handler error:", error);
