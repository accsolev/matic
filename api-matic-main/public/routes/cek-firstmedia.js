const { CekFirstMedia } = require('../../lib/cek-firstmedia-logic.js');
...
function formatFirstMediaResponse(result) {
    if (!result || !result.body?.["Rincian Transaksi"] || !result.top) {
        return { success: false, message: "Struktur data transaksi First Media tidak valid atau tidak ditemukan." };
    }
    ...
    if (sn) {
        ...
        return {
            success: true,
            status: "Sukses",
            message: "Tagihan First Media berhasil ditemukan.",
            bill_details: {
...
module.exports = async function(req, res) {
    try {
        ...
        const firstmediaApi = new CekFirstMedia();
        const result = await firstmediaApi.getBillDetails(customer_id);
        
        const finalResponse = formatFirstMediaResponse(result.details.content_details);
        ...
        console.error("First Media handler error:", error);
