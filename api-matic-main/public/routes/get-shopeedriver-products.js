...
module.exports = async function(req, res) {
    try {
        ...
        if (!auth_token || !auth_username) { return res.status(400).json({ success: false, message: "Parameters 'auth_token' and 'auth_username' are required." }); }
        ...
        if (result.success && result.vouchers?.results) {
            result.vouchers.results = result.vouchers.results.filter(v => v.provider?.name === 'Top Up Saldo Shopee Driver');
        }
