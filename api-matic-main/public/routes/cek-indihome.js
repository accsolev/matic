const { CekIndihome } = require('../../lib/cek-indihome-logic.js');
...
function formatIndihomeResponse(result) {
    if (!result || !result.body?.["Rincian Transaksi"] || !result.top) {
        return { success: false, message: "Struktur data transaksi tidak valid atau tidak ditemukan." };
    }
    ...
    if (sn) {
        ...
        return {
            success: true,
            status: "Sukses",
            message: "Tagihan Indihome berhasil ditemukan.",
            bill_details: {
...
module.exports = async function(req, res) {
    try {
        ...
        const indihomeApi = new CekIndihome();
        // Berdasarkan error Anda, 'result' yang dikembalikan memiliki 'top' dan 'body'.
        const result = await indihomeApi.getBillDetails(customer_id); 
        
        // Kita langsung teruskan 'result.details.content_details' ke formatter
        // karena di situlah data sebenarnya berada.
        const finalResponse = formatIndihomeResponse(result.details.content_details);
        ...
        // Menambahkan log untuk debugging jika terjadi error tak terduga
        console.error("Indihome handler error:", error);
