...
module.exports = async function(req, res) {
    try {
        ...
        if (result.success && result.vouchers?.results) {
            const filteredProducts = result.vouchers.results.filter(voucher => 
                voucher.provider?.name === 'Berlangganan Spotify Premium'
            );
            result.vouchers.results = filteredProducts;
        }
